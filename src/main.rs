...
//    {
//        //String::deserialize(deserializer)?.parse().map_err(D::Error)
...
fn main() {
    println!("Starting!");
    //let target = PackageTarget::from_str("aarch64-darwin").unwrap_or(PackageTarget::from_str("x86_64-linux").expect("FAILED"));
    //let _target = PackageTarget::from_str("aarch64-macos").unwrap();
    //let _target = PackageTarget::from_str("aarch64-darwin").unwrap();
